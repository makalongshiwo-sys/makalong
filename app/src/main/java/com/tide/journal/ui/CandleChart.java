...
public final class CandleChart extends View {
 ...
 @Override protected void onDraw(Canvas c){super.onDraw(c);float w=right()-left(),h=getHeight(),top=24,bottom=h*.47f;
  ...
  label(c,"价格 · USDT",left(),15,Ui.MUTED);label(c,"VOL",left(),h*.52f,Ui.MUTED);label(c,"MACD 12 · 26 · 9",left(),h*.66f,Ui.MUTED);label(c,"RSI 14",left(),h*.84f,Ui.MUTED);
  float vtop=h*.54f,vbottom=h*.62f,mtop=h*.68f,mbottom=h*.80f,mzero=(mtop+mbottom)/2,rtop=h*.86f,rbottom=h-22;
  ...
  label(c,Ui.time(bars.get(begin).openAt),left(),h-3,Ui.MUTED);String time=Ui.time(bars.get(end-1).openAt);p.setTextAlign(Paint.Align.RIGHT);label(c,time,right(),h-3,Ui.MUTED);p.setTextAlign(Paint.Align.LEFT);
 }
